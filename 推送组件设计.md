...
```ts
pusher.publish("*", "a message to all users");
```
...
net.createServer(socket => {
    ...
    pusher.subscribe(`userid_${userId}`, handle);
    for (const topic of topics) {
        pusher.subscribe(`topic_${topic}`, handle);
    }
    pusher.subscribe("*", handle);
    socket.on("close", hadError => {
        pusher.unsubscribe(`userid_${userId}`, handle);
        for (const topic of topics) {
            pusher.unsubscribe(`topic_${topic}`, handle);
        }
