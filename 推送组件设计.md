...
export function subscribeByUserId(userId: string | number, handle: Handle) {
    subscribe(`userid_${userId}`, handle);
}

export function subscribeByGroup(groupName: string, groupId: string | number, handle: Handle) {
    subscribe(`${groupName}_${groupId}`, handle);
}

export function subscribeAll(handle: Handle) {
    subscribe("*", handle);
}
...
export function unsubscribeByUserId(userId: string | number, handle: Handle) {
    unsubscribe(`userid_${userId}`, handle);
}

export function unsubscribeByGroup(groupName: string, groupId: string | number, handle: Handle) {
    unsubscribe(`${groupName}_${groupId}`, handle);
}

export function unsubscribeAll(handle: Handle) {
    unsubscribe("*", handle);
}
...
export function publishToUser(userId: string | number, message: any) {
    publish(`userid_${userId}`, message);
}
...
export function publishToAll(message: any) {
    publish("*", message);
}
```
...
pusher.subscribeByUserId(100, handle1);
pusher.subscribeByGroup("topic", "a", handle1);
pusher.subscribeAll(handle1);
```
...
pusher.subscribeByUserId(101, handle2);
pusher.subscribeByGroup("topic", "a", handle2);
pusher.subscribeAll(handle2);
```
...
pusher.subscribeByUserId(102, handle3);
pusher.subscribeByGroup("topic", "b", handle3);
pusher.subscribeAll(handle3);
```

当数据源希望向用户100推送数据，推送操作如下：

```ts
pusher.publishToUser(100, "a message to user 100 only");
```
...
```ts
pusher.publishToAll("a message to all users");
```
...
net.createServer(socket => {
    ...
    pusher.subscribeByUserId(userId, handle);
    for (const topic of topics) {
        pusher.subscribeByGroup("topic", topic, handle);
    }
    pusher.subscribeAll(handle);
    socket.on("close", hadError => {
        pusher.unsubscribeByUserId(userId, handle);
        for (const topic of topics) {
            pusher.unsubscribeByGroup("topic", topic, handle);
        }
        pusher.unsubscribeAll(handle);
    });
}).listen(8000);
```
