...
缺点：目前小众，开发体验还不够好，编辑器支持程度低，学习曲线太陡了

以js中简单的多阶函数为例：

```js
const sum = (f, g) => x => f(x) + g(x);
const a = sum(x => x * x, x => x * 2 + 1);
console.log(a(1));
console.log(a(2));
console.log(a(3));
```

rust中要考虑所有权，结果代码非常复杂难懂：

```rust
fn sum(f: &'static Fn(i32) -> i32, g: &'static Fn(i32) -> i32) -> Box<Fn(i32) -> i32> {
    Box::new(move |x: i32| f(x) + g(x))
}
let a = sum(&|x: i32| x * x, &|x: i32| x * 2 + 1);
println!("{}", a(1));
println!("{}", a(2));
println!("{}", a(3));
```

rust目前还是需要简化下开发过程，否则开发效率很低。
